...
const toDelete = (id) => {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            router.delete(route("row.destroy", id), {
                onSuccess: (response) => {
                    Swal.fire({
                        title: "Berhasil!",
                    ...
                    getDataPegawaiAlamat(route("row.getdata"));
                },
                onError: (errors) => {
                    if (errors.query) {
                        Swal.fire({
...
watch(paginate, (value) => {
    getDataPegawaiAlamat(
        route("row.getdata") + "?cari=" + cari.value + "&paginate=" + value,
    );
});
...
const showDetail = (id) => {
    getDataAlamatPegawaiDetail(route("row.show", id));
...
<template>
    <div class="overflow-x-auto">
        ...
        <table class="table" aria-describedby="Tabel Alamat Pegawai">
            ...
            <tbody>
                <tr class="hover" v-for="(row, index) in pegawaiAlamat.data">
                    <th scope="col">{{ index + 1 }}</th>
                    <th scope="col">{{ row.nama_lengkap }}</th>
                    <th scope="col">{{ row.tipe_alamat }}</th>
                ...
                <table class="w-11/12">
                    ...
                    <tr>
                        ...
                        <td
                            ...
                            v-html="pegawaiAlamatDetail.nama_lengkap"
                        ></td>
                    </tr>
                    <tr>
                        <td>Tipe Alamat</td>
                    ...
                    <tr>
                        <td>Alamat</td>
                        <td v-html="pegawaiAlamatDetail.row"></td>
                    </tr>
                </table>
            </div>
